...
| `model-exchange-fmu` | `common`, `model-exchange` | `callbacks`
| `co-simulation-fmu` | `common`, `co-simulation` | `callbacks`, `intermediate-update-callbacks` (optional)
| `scheduled-execution-fmu` | `common`, `scheduled-execution` | `callbacks`
...
. Provides all imported interfaces declared in the FMU's world (callbacks, and `intermediate-update-callbacks` when `early-return-allowed = true`).
...
. *WASI imports* — FMUs that need filesystem or clock access will depend on WASI interfaces.
